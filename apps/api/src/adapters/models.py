...
class NormalizedTransaction(BaseModel):
    ...
    token_contract: str | None = Field(
        default=None, description="Smart contract address for token transfers"
    )
    amount: str = Field(
        ..., description="Decimal string preserving full blockchain token precision"
    )
    ...
    provider: str = Field(
        ..., description="Data provider source identifier (e.g., trongrid, llamarpc, demo_sandbox)"
    )
    is_demo: bool = Field(
        default=False, description="Flag explicitly declaring if transaction is demo/mock data"
    )
    raw_reference: dict[str, Any] = Field(
        default_factory=dict,
        description="Raw immutable provider payload for evidentiary provenance",
    )
...
class WalletBalance(BaseModel):
    ...
    token_balances: dict[str, str] = Field(
        default_factory=dict, description="Symbol -> Decimal balance string"
    )
