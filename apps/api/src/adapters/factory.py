...
def get_blockchain_adapter(chain: str | BlockchainType) -> BlockchainAdapter:
    ...
    try:
        chain_enum = BlockchainType(chain.lower().strip() if isinstance(chain, str) else chain.value)
    ...
    if chain_enum not in _adapters:
        ...
        elif chain_enum in (BlockchainType.ETHEREUM, BlockchainType.BINANCE_SMART_CHAIN, BlockchainType.POLYGON):
