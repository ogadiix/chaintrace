...
class VaspEntity(BaseModel):
    ...
    status: str = Field(
        default="ACTIVE", description="'ACTIVE', 'SANCTIONED', 'DEFUNCT', 'INVESTIGATION_TARGET'"
    )
...
class WalletLabel(BaseModel):
    ...
    label_type: str = Field(
        default="DEPOSIT",
        description="'HOT_WALLET', 'COLD_WALLET', 'DEPOSIT', 'ROUTER', 'SANCTIONED'",
    )
    confidence: AttributionConfidence = Field(default=AttributionConfidence.CONFIRMED)
    source: str = Field(
        ..., description="Source provenance e.g. 'PUBLIC_DISCLOSURE', 'OFAC', 'DEMO_DATA'"
    )
    source_reference: str | None = Field(
        default=None, description="URL or documentary reference ID"
    )
...
class WalletAttribution(BaseModel):
    ...
    status: AttributionStatus = Field(
        ..., description="'MATCHED', 'CONFLICTING_LABELS', or 'UNKNOWN'"
    )
    ...
    attributed_via: str = Field(
        default="NONE", description="'EXACT_MATCH', 'PATH_ENDPOINT', 'CLUSTER', 'NONE'"
    )
